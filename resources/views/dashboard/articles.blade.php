...
async function loadModalFilters() {
    const [cats, tags] = await Promise.all([
        fetch('/api/v1/categories',{headers:{Accept:'application/json'}}).then(r=>r.json()).catch(()=>({data:[]})),
        fetch('/api/v1/tags',{headers:{Accept:'application/json'}}).then(r=>r.json()).catch(()=>({data:[]})),
    ]);
...
async function loadMediaPicker() {
    try {
        const r = await fetch('/api/v1/media?per_page=30',{headers:{Accept:'application/json'}});
